...
public class PanAndZoom : MonoBehaviour {
    ...
    void Update() {
        ...
        if (useMouse && canUseMouse) {
            UpdateWithMouse(Input.mouseScrollDelta,mouseScrollSpeed);
        } else {
            UpdateWithTouch();
        }
    }
    ...
    void UpdateWithMouse(Vector2 delta,float mouseScrollSpeeds) {
        ...
        if (delta.y != 0) {       
            OnPinch(Input.mousePosition, 1, delta.y < 0 ? (1 / mouseScrollSpeeds) : mouseScrollSpeeds, Vector2.right);
        }
    }
    ...
    void Zoom(float step)
    {
        ...
        //UpdateWithMouse(new Vector2(0, step),mouseScrollSpeed/1000f);
    ...
    void OnPinch(Vector2 center, float oldDistance, float newDistance, Vector2 touchDelta) {
        ...
        if (controlCamera && cameraControlEnabled) {
            ...
            if (cam.orthographic) {
                ...
                size = Mathf.Max(0.1f, cam.orthographicSize * oldDistance / newDistance);
                cam.orthographicSize = Mathf.Clamp(size, minSize, maxSize);
    ...
    void CameraInBounds() {
        if(controlCamera && useBounds && cam != null && cam.orthographic) {
            size = Mathf.Min(cam.orthographicSize, ((boundMaxY - boundMinY) / 2) - 0.001f);
            cam.orthographicSize = Mathf.Clamp(size, minSize, maxSize);
            size = Mathf.Min(cam.orthographicSize, (Screen.height * (boundMaxX - boundMinX) / (2 * Screen.width)) - 0.001f);
            cam.orthographicSize = Mathf.Clamp(size, minSize, maxSize);
